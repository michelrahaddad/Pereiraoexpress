...
import { prismaStorage } from "../repositories/prisma-storage";
import { isPrismaAuthenticated, setupPrismaAuth } from "../auth/prisma-auth";
  ...
  if (existing.length === 0) {
    ...
    for (const mat of materials) {
      await storage.createMaterial({
        ...
        salePrice: Math.round(mat.costPrice * 1.3),
...
export async function registerPrismaRoutes(
  httpServer: Server,
  app: Express
): Promise<Server> {
  setupPrismaAuth(app);
  ...
  app.get("/api/provider/wallet", isAuthenticated, async (req: any, res) => {
    try {
      ...
      res.json({
        ...
        bankData: userData ? {
          pixKeyType: userData.pix_key_type,
          pixKey: userData.pix_key,
          bankName: userData.bank_name,
          bankAgency: userData.bank_agency,
  ...
  app.post("/api/provider/withdraw", isAuthenticated, async (req: any, res) => {
    try {
      ...
      const userData = await prisma.users.findFirst({ where: { id: userId } });
      if (!userData?.pix_key || !userData?.pix_key_type) {
  ...
  app.get("/api/admin/stats", isAuthenticated, async (req, res) => {
    try {
      ...
      res.json({
        ...
        totalProviders: Math.floor(usersCount * 0.3),
...
Baseie seu diagnóstico no que você vê na imagem combinado com a descrição do cliente.` });
  ...
  app.get("/api/providers/available", isAuthenticated, async (req: any, res) => {
    try {
      ...
      const providersWithPricing = filteredProviders.map(provider => {
        ...
        let priceMax = priceRangeMin + 10000;
        
        if (totalRatings === 0) {
          priceMin = priceRangeMin;
          priceMax = priceRangeMin + 10000;
        } else if (ratingLevel === "Premium") {
          const ratingFactor = (rating - 9) / 1;
          const baseOffset = Math.round(priceRange * 0.35);
          const ratingBonus = Math.round(priceRange * 0.15 * ratingFactor);
  ...
  app.post("/api/reviews", isAuthenticated, async (req: any, res) => {
    try {
      ...
      if (providerProfile) {
        const currentRating = parseFloat(providerProfile.rating || "10");
        const totalRatings = providerProfile.totalRatings || 0;
  ...
  app.get("/api/admin/users", isAuthenticated, isAdmin, async (req, res) => {
    try {
      const results = await prisma.user_profiles.findMany({
      ...
      const enriched = results.map(r => {
        ...
        for (const key of Object.keys(r)) {
          ...
          profile[camelKey] = r[key as keyof typeof r];
        }
        return {
          ...profile,
          email: r.users?.email,
  ...
  app.patch("/api/admin/users/:userId", isAuthenticated, isAdmin, async (req: any, res) => {
    try {
      ...
      if (Object.keys(userUpdates).length > 0) {
        await prisma.users.update({ where: { id: userId }, data: userUpdates });
      }
...
${guidedAnswers ? `Respostas adicionais: ${JSON.stringify(guidedAnswers)}` : ""}`;
  ...
  app.get("/api/service/:id/full", isAuthenticated, async (req, res) => {
    try {
      ...
      if (service.providerId) {
        const providerProfile = await storage.getUserProfile(service.providerId);
        const providerUser = await prisma.users.findFirst({ where: { id: service.providerId } });
  ...
  app.patch("/api/user/settings", isAuthenticated, async (req: any, res) => {
    try {
      ...
      const updatedUser = await prisma.users.findFirst({ where: { id: userId } });
      ...
      res.json({ 
        ...
        user: updatedUser ? {
          ...
          cpf: updatedUser.cpf,
          age: updatedUser.age,
          pixKeyType: updatedUser.pix_key_type,
          pixKey: updatedUser.pix_key,
          bankName: updatedUser.bank_name,
          bankAgency: updatedUser.bank_agency,
          bankAccount: updatedUser.bank_account,
  ...
  app.post("/api/twilio/call", isAuthenticated, async (req: any, res) => {
    try {
      ...
      const providerData = await prisma.users.findFirst({ where: { id: providerId } });
      if (!providerData || !providerData.phone) {
